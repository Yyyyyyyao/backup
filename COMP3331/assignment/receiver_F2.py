...
def main():
	...
	while True:
		...
		if (state == STATE_END):
			...
			header = "rcv"
			pkt_type = 'A'
			write_to_log_file(receiver_log, pkt_receive, header, pkt_type, start_time)
			...
			header = "snd"
			pkt_type = 'A'
			write_to_log_file(receiver_log,  pkt_1, header, pkt_type, start_time)
			...
			header = "snd"
			pkt_type = 'F'
			write_to_log_file(receiver_log, pkt_2, header, pkt_type, start_time)
			...
			dup_sent = False
			ack = int(get_ack(pkt_receive))

			if(checksum_data != checksum_header and len(data) != 0):
				...
				if(is_ack(pkt_receive)):
					...
					if(not dup_sent):
						flag = 0b010
						pkt = packet(ack, cumulated_ack, '', flag, -2)
						receiverSocket.sendto(str(pkt), addr)
						if(dup_ack == int(get_ack(pkt))):

							header = "snd/DA"
							pkt_type = 'A'
							write_to_log_file(receiver_log, pkt, header, pkt_type, start_time)
							'''			    			
							curr_time = float("{:6.2f}".format(time.time()*1000 - start_time))
...
def write_to_log_file(receiver_log, pkt, header, pkt_type, start_time):
