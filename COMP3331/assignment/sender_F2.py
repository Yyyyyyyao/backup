...
def fourseg_termination(ack, seq, senderSocket, addr, sender_log, start_time):
	...
	header = "snd"
	pkt_type = 'F'
	write_to_log_file(sender_log, pkt_1, header, pkt_type, start_time)
	...
	while True:
		try:
			...
			if(is_fin(pkt_receive)):
				...
				header = "rcv"
				pkt_type = 'F'
				write_to_log_file(sender_log, pkt_receive, header, pkt_type, start_time)
				...
				header = "snd"
				pkt_type = 'A'
				write_to_log_file(sender_log, pkt_2, header, pkt_type, start_time)
...
def PLD(sample_timer, pkt, start_time, senderSocket, addr, sender_log, pDrop, pDuplicate, pCorrupt, pOrder, maxOrder, pDelay, maxDelay, seed, re_order_window, re_order_counter, retransmit_flag):
	...
	if(len(delay_window) > 0):
		for pkt in delay_window:
			if(time.time()*1000-pkt[1] > pkt[2]):
				senderSocket.sendto(str(pkt[0]), addr)

		delay_window[:] = [pkt for pkt in delay_window if time.time()*1000-pkt[1] < pkt[2]]
	'''
	...
	if(len(re_order_window) > 0):
		if(re_order_counter == maxOrder):
			...
			header = log_header+"/rord"
			pkt_type = 'D'
			write_to_log_file(sender_log, re_order_window[0], header, pkt_type, start_time)
			'''
			curr_time = float("{:6.2f}".format(time.time()*1000-start_time))
	...
	if(ran < pDrop):
		...
		header = "drop"
		pkt_type = 'D'
		write_to_log_file(sender_log, pkt, header, pkt_type, start_time)
		'''
		curr_time = float("{:6.2f}".format(time.time()*1000-start_time))
		...
		if(ran < pDuplicate):
			...
			if(ran < pCorrupt):
				...
				header = log_header+"\corr"
				pkt_type = 'D'
				write_to_log_file(sender_log, newpkt, header, pkt_type, start_time)
				'''
				curr_time = float("{:6.2f}".format(time.time()*1000-start_time))
				...
				if(ran < pOrder):
					...
					if(ran < pDelay):
						...
						#time.sleep(delay)
						delay_pkt = []
						delay_pkt.append(pkt)
						delay_pkt.append(time.time()*1000)
						delay_pkt.append(delay)
						delay_window.append(delay_pkt)

						senderSocket.sendto(str(pkt), addr)
						header = log_header+"/dely"
						pkt_type = 'D'
						write_to_log_file(sender_log, pkt, header, pkt_type, start_time)
						'''
						curr_time = float("{:6.2f}".format(time.time()*1000-start_time))
...
def transfer(ack, ini_seq, seq, senderSocket, addr, data, sender_log, start_time, mws, mss, pDrop, pDuplicate, pCorrupt, pOrder, maxOrder, pDelay, maxDelay, seed, gamma):
	...
	while(True):
		...
		if(time.time()*1000 - baseTime >= TimeoutInterval and len(window) > 0):
			...
			#delay_window = pld_list[3]

		# add paccket into window 
...
def write_to_log_file(sender_log, pkt, header, pkt_type, start_time):

	curr_time = time.time()*1000 - start_time
	time_stamp = round(curr_time/1000, 3)

	args = [header, time_stamp, pkt_type, str(get_seq(pkt)), str(len(get_data(pkt))), str(get_ack(pkt))]
	log_data = "{0:<10}{1:<10.4}{2:<10}{3:<10}{4:<10}{5:<10}\n".format(*args)
	sender_log.write(log_data)
