...
def transfer(ini_seq, seq, senderSocket, addr, data, sender_log, start_time, mws, mss, pDrop, pDuplicate, pCorrupt, pOrder, maxOrder, pDelay, maxDelay, seed, gamma):
	...
	while(True):
		...
		if(time.time()*1000 - baseTime >= TimeoutInterval and len(window) > 0):
			...
			re_order_pkt = PLD(pkt, start_time, senderSocket, addr, sender_log, pDrop, pDuplicate, pCorrupt, pOrder, maxOrder, pDelay, maxDelay, seed, re_order_window, re_order_counter, retransmit_flag)
...
if __name__ == "__main__":
	...
	seq = threeway_handshake(seq, senderSocket, addr, start_time, seed, sender_log)
	...
	seq = transfer(ini_seq, seq, senderSocket, addr, data, sender_log, start_time, mws, mss, pDrop, pDuplicate, pCorrupt, pOrder, maxOrder, pDelay, maxDelay, seed, gamma)
	...
	# senderSocket.close()
	fourseg_termination(seq, senderSocket, addr, sender_log, start_time)
