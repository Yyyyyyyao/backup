...
def main():
	...
	transfer(senderSocket, addr, data, sender_log, start_time, mws, mss)
...
def transfer(senderSocket, addr, data, sender_log, start_time, mws, mss):
	...
	while(True):
		...
		while(not window or (len(window) < window_size and seq < len(data))):
			...
			# timeout 
			if(curr_time - baseTime >= TimeoutInterval and len(window) > 0 and int(windowBase) == baseWindowSeq):
			...
			flag = 0b000
			pkt = packet(seq, 1, pkt_data, flag)
		...
		try:
			...
			if(is_ack(pkt_receive)):
				...
				window[:] = [pkt for pkt in window if int(get_seq(pkt))+int(len(get_data(pkt))) > pre_ack]
